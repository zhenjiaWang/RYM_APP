define(function(require, exports, module) {
	...
	bindEvent = function() {
		$common.touchSE($('.personBoard', '#commentDIV'), function(event, startTouch, o) {}, function(event, o) {
			...
			if (productId) {
				$windowManager.create('product_commentFooter', '../product/commentFooter.html?id=' + productId + '&userId=' + $userInfo.get('userId'), false, true, function(show) {
					show();
				});
			}
		});
	...
	loadData = function(callback, append) {
		...
		$.ajax({
			type: 'POST',
			url: $common.getRestApiURL() + '/product/info/commentList',
			dataType: 'json',
			data: {
